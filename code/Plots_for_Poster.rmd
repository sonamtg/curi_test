...
stor_zoomed <- crop(nk_cor, extent(void_zoomed)) %>% trim()
plot(stor_zoomed, col = rwb1, zlim = c(-15,15), 
     main = "Before Bilinear", xaxt = "n", yaxt = "n")
par(bg = "darkgray")
interpolated_zoomed <- crop(fill_raster, extent(void_zoomed)) %>% trim()
plot(interpolated_zoomed,  col = rwb1, zlim = c(-15,15), 
     main = "After Bilinear", xaxt = "n", yaxt = "n", legend = FALSE)

```

#Krig plot
```{r}
#refer to Krig_6th for more info;
...
stor_void_na <- stor_void_na %>% 
  mutate(krig_elev_diff = var1.pred *pred_mad + pred_med_elev_diff)
...
interpolated_zoomed<- crop(fill_raster, extent(void_zoomed)) %>% trim()
plot(interpolated_zoomed,  col = rwb1, zlim = c(-15,15), 
     main = "Hypsometric", xaxt = "n", yaxt = "n", legend = FALSE)
...
interpolated_zoomed<- crop(fill_raster, extent(void_zoomed)) %>% trim()
plot(interpolated_zoomed,  col = rwb1, zlim = c(-15,15), 
     main = "Bilinear", xaxt = "n", yaxt = "n", legend = FALSE)
...
#plot(stor_rast, col = rwb1, zlim = c(-15,15), main = "Kriging", xaxt = "n", yaxt = "n", legend = FALSE)

krig_zoomed<- crop(stor_rast, extent(void_zoomed)) %>% trim()
plot(krig_zoomed,  col = rwb1, zlim = c(-15,15), 
     main = "Kriging", xaxt = "n", yaxt = "n", legend = FALSE)
```
...
#coreg
pdf("/home/rstudio/users/gurung2/curi_test/images/coreg.pdf", width = 6.5, height = 4)
dev.off()

#hypso
pdf("/home/rstudio/users/gurung2/curi_test/images/hypo.pdf", width = 6.5, height = 4)
dev.off()

#bilinear
pdf("/home/rstudio/users/gurung2/curi_test/images/bilinear.pdf", width = 6.5, height = 4)
dev.off()

#krig
pdf("/home/rstudio/users/gurung2/curi_test/images/krig.pdf", width = 6.5, height = 4)
dev.off()

#all interpolation 

pdf("/home/rstudio/users/gurung2/curi_test/images/all_inter.pdf", width = 6.5, height = 4)
dev.off()

#coreg compare 
pdf("/home/rstudio/users/gurung2/curi_test/images/coreg_compare.pdf", width = 6.5, height = 4)
dev.off()

#coreg zoomed in 

pdf("/home/rstudio/users/gurung2/curi_test/images/coreg_zoomed", width = 6.5, height = 4)
dev.off()

#slope and nmad box plot 

pdf("/home/rstudio/users/gurung2/curi_test/images/slope_nmad.pdf", width = 6.5, height = 4)
dev.off()
...
void_zoomed <- c(648200, 648800, 7536500, 7539000)
stor_zoomed<- crop(nk_cor, extent(void_zoomed)) %>% trim()
plot(stor_zoomed, col = magma(10), zlim = c(-20,20), main = "Before Interpolation")

interpolated_zoomed<- crop(fill_raster, extent(void_zoomed)) %>% trim()
plot(interpolated_zoomed,  col = magma(10), zlim = c(-20,20), main = "After Interpolation")


#RWB version
rwb1 <- colorRampPalette(c("red", "white", "blue"))(20)
par(mfrow= c(1,2))
void_zoomed <- c(648200, 648800, 7536500, 7539000)
stor_zoomed<- crop(nk_cor, extent(void_zoomed)) %>% trim()
plot(stor_zoomed, col = rwb1, zlim = c(-15,15), main = "Before Interpolation")
par(bg = "darkgray", add = TRUE)
interpolated_zoomed<- crop(fill_raster, extent(void_zoomed)) %>% trim()
plot(interpolated_zoomed,  col = rwb1, zlim = c(-15,15), main = "After Interpolation")

# #section of void 
# stor_void_zoomed <- stor_cor_glacier %>% 
#   filter(x >= 648200, x <= 648800 & y >= 7536500, y<= 7539000)
# 
# subset_extent3 <- c(648200, 648400, 7536500, 7539000)
# subset_extent4 <- c(648400,648600, 7536500, 7536800)
# subset_extent5 <- c(648600, 648800, 7536500, 7539000)
# par(mfrow= c(1,2))
# plot(trim(nk_cor), col = rwb1, zlim = c(-15,15))
# plot(extent(c(subset_extent5)), col = "green", add = TRUE)
# ddemzoom_cropped <- crop(nk_cor, extent(subset_extent5))%>%
#   trim()
# plot(ddemzoom_cropped, main = "Cor",  col = rwb1, zlim = c(-2.5,2.5))
